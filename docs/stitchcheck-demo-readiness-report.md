...
|---|---|---|---|
| **Gemini** | GEM-01 executed via OpenRouter temporary path (`google/gemini-3.7-flash`). Labelled `OpenRouter temporary path — not direct Gemini validation`. Human-reviewed and confirmed offline. Raw response redacted. | **No.** Direct Gemini has not been executed. Pass/fail for direct Gemini is intentionally blank. | Direct Gemini execution with confirmed model/API configuration, structured response evidence, and pass/fail decision per `docs/smoke-test-execution-checklist.md`. |
| **Nosana** | Local fixtures (`req-nos-*`, `res-nos-*`), schema validator, and workload skeleton only. All labelled `Synthetic local placeholder — not Nosana evidence`. Zero network code. Zero deployment. Job definition corrected to match official Nosana schema v0.1 (`version`, `type`, `ops[]`, `createNosanaClient()` SDK init, official-schema `validateJobDefinition()`). Offline tests: 75 passed, 0 failed. A smoke-test attempt (NOS-ATTEMPT-001) was **intentionally blocked before any network request** on 2026-08-20T15:53:43Z. No paid workload has been submitted. | **No.** Nosana has not been executed, deployed, or authenticated. | `@nosana/kit` SDK must be installed; market address must be verified via read-only API call; credit account required; human authorization for one paid workload required. |
...
- Direct Gemini has passed. (Direct Gemini validation remains `not_executed`.)
- Nosana works or has been deployed. (Nosana has not been executed, deployed, or authenticated. The smoke-test attempt was blocked before any network request due to missing infrastructure. The blocked result is valid evidence of a safe stop, not provider evidence.)
- Atlas production search has been executed via the official Skill CLI. (Atlas authentication succeeded; two live production searches returned real offers. Atlas Sandbox was not used. All offers are reference-price only with ticketing activation pending.)
