...
> **Provider transition (2026-08-29):** Gemini extraction was replaced by **MiniMax M3 via OpenRouter** (`minimax/minimax-m3:free`). The current live extraction provider is MiniMax M3; Gemini rows below are retained only as historical artifacts. Current evidence: `demo-evidence/2026-08-29-submission-final/00-report.txt`.

## Provider Evidence

| Capability | Status | Proof / Artifact | Wording Allowed in Video | Wording NOT Allowed |
|---|---|---|---|---|
| **MiniMax M3 extraction (via OpenRouter)** — current provider | Live extraction validated — schema-validated, `fallbackUsed: false` | `demo-evidence/2026-08-29-submission-final/00-report.txt`, `smoke-tests/extraction/` | "MiniMax M3 via OpenRouter — live extraction validated." "Extracted by MiniMax M3." | "Gemini extracted this", "Extraction skipped or fallback" |
| **Gemini extraction (direct 3.7)** | Historical — replaced by MiniMax M3 via OpenRouter on 2026-08-29; live extraction was previously validated | `smoke-tests/gemini/results/results-gemini-3.7-flash-success.json` (historical artifact) | "Gemini extraction was previously validated; the current extraction provider is MiniMax M3 via OpenRouter." | "Direct Gemini — live validated" as a current-state claim, "Gemini is the current provider" |
| **Gemini extraction (direct 3.6)** | Historical — replaced by MiniMax M3 via OpenRouter | Previous evidence package | "Gemini extraction was previously validated." | "Gemini 3.6 is the current provider" |
...
|---|---|---|
| `EXTRACTION_PROVIDER_LABELS.liveExtraction` (alias `GEMINI_LABELS`, deprecated) | `Source: AI extraction (MiniMax M3 via OpenRouter) · live` | When extraction has `evidenceSource: 'extraction-live'`, `provider: 'openrouter'`, `executed: true`, `fallbackUsed: false`. The UI provenance tag `Extracted by MiniMax M3` is derived from the same real extraction state. |
...
4. `OPENROUTER_API_KEY` is server-side only and never appears in the browser bundle.
...
9. The live extraction label requires ALL of: `evidenceSource: 'extraction-live'`, `provider: 'openrouter'`, `executed: true`, `fallbackUsed: false`.
...
|---|---|---|
| MiniMax M3 via OpenRouter (current) | Live extraction validated | "MiniMax M3 via OpenRouter — live extraction validated; schema-validated, `fallbackUsed: false`." |
| Direct Gemini 3.7 (historical) | Live extraction previously validated; replaced by MiniMax M3 | "Gemini extraction was previously validated (historical artifact); the current extraction provider is MiniMax M3 via OpenRouter." |
...
|---|---|---|---|
| MiniMax M3 via OpenRouter (current) | Fictional itinerary image | Live, schema-validated | MiniMax M3 via OpenRouter — live extraction validated |
